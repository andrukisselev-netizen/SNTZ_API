...
- **Модели:** gemini-2.5, gemini-3.1 (поддержка до 3/5 входных изображений);
...
### Вариант 1: плагин Photoshop (UXP)
...
### Вариант 2: JSX-скрипты (PsUI)

1. В Photoshop: **File → Scripts → Browse** → `export-layer-as-linked-palette.jsx`.
2. В палитре указываете модель и промпт, нажимаете **Generate**.
3. Триггер отправляет workflow в ComfyUI, результат перезаписывается в папку экспорта.
4. **Update Modified Content** выполняется автоматически.
...
1. Установите **ComfyUI** и **ComfyUI-Manager** (по желанию).
2. Добавьте **SNTZ_API** в `custom_nodes/` (через Manager или `git clone`).
