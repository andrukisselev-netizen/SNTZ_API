...
| `workflow_ps_linked_api.json` | Шаблон workflow для API. **API ключ:** плагин UXP берёт `api_key` из этого файла (нода SNTZPSLinkedFolderFlux). Чтобы сменить ключ — отредактируйте файл или в ComfyUI измените ключ в ноде и сохраните workflow в этот файл. |
